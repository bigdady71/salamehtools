...
<head>
    ...
    <title>Cash Refund Approvals - Admin</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>Cash Refund Approvals</h1>
                <p style="color:#666; margin-top:4px;">Review and approve customer cash refund requests</p>
            </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div style="font-size:0.85rem; color:#64748b;">$<?= number_format((float)($totals['pending_usd'] ?? 0), 2) ?></div>
            </div>
            <div class="stat-card">
                <div class="label">Approved (Awaiting Payment)</div>
                <div class="value approved"><?= (int)($totals['approved_count'] ?? 0) ?></div>
                <div style="font-size:0.85rem; color:#64748b;">$<?= number_format((float)($totals['approved_usd'] ?? 0), 2) ?></div>
            </div>
            <div class="stat-card">
                ...
                <div class="return-card <?= $isPending ? 'pending' : ($isApproved ? 'approved' : '') ?>">
                    ...
                    <div class="return-details">
                        <div class="detail-item">
                            ...
                            <div style="font-size:0.8rem; color:#64748b;"><?= htmlspecialchars($return['customer_phone'] ?? '') ?></div>
                        </div>
                        <div class="detail-item">
                        ...
                        <?php if ($return['approved_by_name']): ?>
                            <div class="detail-item">
                                ...
                                <div style="font-size:0.8rem; color:#64748b;"><?= date('M j, Y', strtotime($return['approved_at'])) ?></div>
                            </div>
                        <?php endif; ?>
                        ...
                        <?php if ($return['paid_by_name']): ?>
                            <div class="detail-item">
                                ...
                                <div style="font-size:0.8rem; color:#64748b;"><?= date('M j, Y', strtotime($return['cash_paid_at'])) ?></div>
                            </div>
                        <?php endif; ?>
                    </div>

                    <?php if ($return['notes']): ?>
                        <div style="background:#f9fafb; padding:12px; border-radius:8px; margin-bottom:16px; font-size:0.9rem;">
                        ...
                        html += `
                            <div class="item">
                                <div>
                                    ...
                                    <small style="color:#64748b;">${escapeHtml(item.sku)} x ${item.quantity}</small>
