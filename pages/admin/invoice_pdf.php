...
/**
 * Admin Invoice PDF viewer/generator
 * Allows admins to view, download, or share any invoice as PDF
 */
...
// Get invoice ID from query parameter - support both 'id' and 'invoice_id'
$invoiceId = isset($_GET['id']) ? (int)$_GET['id'] : (isset($_GET['invoice_id']) ? (int)$_GET['invoice_id'] : 0);
...
try {
    // For admin users, we pass null as user_id to bypass ownership check
    // The InvoicePDF class should allow full access when user_id is null
    $adminUserId = null; // Admin can view any invoice

    switch ($action) {
        case 'download':
            // Stream PDF as download
            $pdfGenerator->streamPDF($invoiceId, $adminUserId, true);
            break;

        case 'save':
            // Save PDF to storage and return path
            $filepath = $pdfGenerator->savePDF($invoiceId, $adminUserId);
            if ($filepath) {
        ...
        case 'whatsapp':
            ...
            $invoice = $pdfGenerator->getInvoiceData($invoiceId, $adminUserId);
            if (!$invoice) {
                http_response_code(404);
            ...
            $pdfUrl = $baseUrl . '/pages/admin/invoice_pdf.php?id=' . $invoiceId . '&action=download';
            ...
            $centsDiscount = $invoice['cents_discount'] ?? 0;
        ...
        default:
            // View PDF in browser
            $pdfGenerator->streamPDF($invoiceId, $adminUserId, false);
            break;
    }
} catch (Exception $e) {
    http_response_code(500);
