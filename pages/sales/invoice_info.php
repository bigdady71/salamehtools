 ...
 * Sales rep version - only returns invoices for their assigned customers.
 */
...
// Get invoice - only if it belongs to the sales rep's customer
$stmt = $pdo->prepare("
    SELECT
        i.id,
        i.invoice_number,
    ...
    WHERE i.id = ? AND c.assigned_sales_rep_id = ?
");
$stmt->execute([$invoiceId, $repId]);
...
$requestStmt = $pdo->prepare("
    ...
    LIMIT 1
");
$requestStmt->execute([$invoiceId]);
$invoice['pending_request'] = $requestStmt->fetch(PDO::FETCH_ASSOC) ?: null;
