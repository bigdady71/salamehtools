...
<?php if ($hasStockIssues): ?>
    <div class="alert alert-warning">
        <strong>⚠️ Stock Issue:</strong> Some items have insufficient stock. Please return to your cart and adjust quantities before proceeding.
    </div>
<?php endif; ?>
            ...
            <form method="post" action="checkout.php" id="checkoutForm">
                ...
                <div class="form-group">
                    ...
                    <textarea
                        id="notes"
                        name="notes"
                        placeholder="Add any special instructions or notes for your sales representative..."
                    ></textarea>
                </div>

                <div style="padding: 16px; background: var(--bg-panel-alt); border-radius: 10px; border: 1px solid var(--border); margin-bottom: 20px;">
        ...
        <div class="card" style="position: sticky; top: 24px;">
            ...
            <div style="margin-top: 24px; padding: 16px; background: var(--bg-panel-alt); border-radius: 10px; border: 1px solid var(--border);">
                ...
                <div style="font-size: 0.85rem; color: var(--muted); line-height: 1.6;">
                    <p style="margin: 0 0 8px;"><strong>Location:</strong><br><?= htmlspecialchars($customer['location'] ?? 'Not specified', ENT_QUOTES, 'UTF-8') ?></p>
                    <p style="margin: 0;"><strong>Phone:</strong><br><?= htmlspecialchars($customer['phone'] ?? 'Not specified', ENT_QUOTES, 'UTF-8') ?></p>
                </div>
            </div>

            <?php if ($customer['sales_rep_name']): ?>
                <div style="margin-top: 16px; padding: 16px; background: var(--bg-panel-alt); border-radius: 10px; border: 1px solid var(--border);">
                    <h3 style="margin: 0 0 12px; font-size: 0.95rem; color: var(--text);">Your Sales Rep</h3>
                    <div style="font-size: 0.85rem; color: var(--muted); line-height: 1.6;">
                        <p style="margin: 0 0 4px;"><strong><?= htmlspecialchars($customer['sales_rep_name'], ENT_QUOTES, 'UTF-8') ?></strong></p>
                        <?php if ($customer['sales_rep_phone']): ?>
                            <p style="margin: 0;">📞 <?= htmlspecialchars($customer['sales_rep_phone'], ENT_QUOTES, 'UTF-8') ?></p>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endif; ?>
